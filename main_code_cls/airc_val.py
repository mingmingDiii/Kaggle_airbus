...
def make_loader(mode,img_size,batch_size,shuffle=False):
    return DataLoader(
        dataset=data_load.AIRBUS_CLASS(mode=mode,img_size=img_size,if_name=True),
        shuffle=shuffle,
        num_workers=4,
...
trans_df = pd.read_csv('/mnt/sda1/don/documents/airbus/data/v2/split_list/val.csv',header=None)
trans_df.columns = ['ImageId']
val_loader = make_loader(mode='valall',img_size=IMAGE_SIZE,batch_size=batch_size)

init_path ='/mnt/sda1/don/documents/airbus/main_code_cls/experiments/densenet121/weights/airbus_cls_26'
...
with  torch.no_grad():
    ...
    trans_df[['ImageId','score']].to_csv('val_score.csv',index=False)

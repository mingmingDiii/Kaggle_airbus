...
class AirbusDS(data.Dataset):
    ...
    def get_mask(self, ImageId):
        ...
        for mask in img_masks:
            ...
            in_mask_point = np.where(decode_mask)
            link_label[in_mask_point]=1
...
if __name__ == '__main__':
    ...
    airimg = AirbusDS('valmore', img_size=384,transform=None)
    ...
    for x in range(100):
        ...
        plt.subplot(337)
        plt.imshow(masks[0,6,:,:])
        plt.subplot(338)
        plt.imshow(masks[0,7,:,:])
        plt.subplot(339)
